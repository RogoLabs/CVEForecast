...
class ForecastTracker:
    ...
    def __init__(self, history_path: str = 'web/forecast_history.json'):
        ...
        logger.info(
            f'Initialized ForecastTracker with {len(self.history.get("forecast_snapshots", []))} existing snapshots'
        )
    ...
    def _update_accuracy_tracking(self):
        ...
        for month, actual in months_with_actuals.items():
            ...
            if forecast_evolution:
                ...
                volatility = (
                    np.std(all_forecasts) / np.mean(all_forecasts) * 100
                    if len(all_forecasts) > 0 and np.mean(all_forecasts) != 0
                    else 0
                )
    ...
    def get_model_stability_ranking(self) -> List[tuple]:
        ...
        ranking = [(model, data['stability_score']) for model, data in metrics.items()]
