...
try:
    import lightgbm as lgb  # noqa: F401 - imported for side-effect env var setup
...
class DynamicTimeoutManager:
    ...
    def model_completed(self, model_name: str, success: bool = True):
        ...
        if remaining_models > 0:
            ...
            print(
                f'🔄 Dynamic timeout: {new_timeout / 60:.1f} minutes per remaining model ({remaining_models} models left)'
            )
...
class ComprehensiveHyperparameterTuner:
    ...
    def load_data(self) -> TimeSeries:
        ...
        series = TimeSeries.from_dataframe(monthly_counts, freq='M', fill_missing_dates=True, value_cols='cve_count')
    ...
    def generate_parameter_combinations(
        self, model_name: str, search_type: str = 'grid', max_combinations: Optional[int] = None
    ) -> List[Dict]:
        ...
        for split_ratio in split_ratios:
            for param_combo in param_combinations:
                full_combinations.append({'split_ratio': split_ratio, 'hyperparameters': param_combo})
    ...
    def test_model_viability(
        self, model_name: str, model_config: Dict, series: TimeSeries, timeout_seconds: float = 30
    ) -> bool:
        """
    ...
    def tune_model_comprehensive(
        ...
        max_consecutive_failures = (
            50 if model_name in ['LinearRegression', 'AutoARIMA', 'ExponentialSmoothing', 'Prophet'] else 25
        )  # Increased for problematic models
        ...
        for trial_num, combo in enumerate(combinations, 1):
            ...
            if timeout_seconds and elapsed_model_time >= (timeout_seconds * 0.95):
                timeout_triggered = True
                print(
                    f'  ⏱️ {model_name} approaching timeout ({elapsed_model_time:.1f}s/{timeout_seconds:.1f}s) - stopping trials'
                )
                break
            ...
            if trial_num % progress_interval == 0 or trial_num == total_combinations:
                ...
                if timeout_seconds:
                    ...
                    if remaining <= 0:
                        ...
                        break
                    print(
                        f'  ⚡ Progress: {progress:.1f}% ({trial_num:,}/{total_combinations:,}) - {remaining:.0f}s remaining',
                        end='\r',
                        flush=True,
                    )
                else:
                    print(
                        f'  ⚡ Progress: {progress:.1f}% ({trial_num:,}/{total_combinations:,}) - {elapsed:.0f}s elapsed',
                        end='\r',
                        flush=True,
                    )
            ...
            try:
                ...
                if model and predictions:
                    ...
                    if consecutive_failures >= max_consecutive_failures:
                        print(
                            f'\r  ❌ {model_name} has {consecutive_failures} consecutive failures - stopping early'
                            + ' ' * 20
                        )
                        self.failed_models.add(model_name)
                        self.save_failed_models()  # Persist the failed model
        ...
        if timeout_triggered:
            ...
            print(
                f'  📊 Completed {len(results)}/{total_combinations} trials ({len(results) / total_combinations * 100:.1f}%)'
            )
    ...
    def show_search_space_overview(
        self,
        ...
        for space in search_spaces:
            ...
            print(
                f'{space.model_name:<20} {space.total_combinations:<12,} '
        ...
        if limited_count > 0:
            ...
            print('🎲 Random search is recommended for limited space exploration')
        else:
    ...
    def run_comprehensive_tuning(
        ...
        if search_config.timeout_minutes is not None:
            ...
            dynamic_timeout_manager = DynamicTimeoutManager(
                total_budget_minutes=search_config.timeout_minutes, active_models=active_model_names
            )
            print(
                f'🔄 Dynamic timeout enabled: {search_config.timeout_minutes:.1f} minutes total budget across {len(active_model_names)} models'
            )
        ...
        for i, (model_name, model_config) in enumerate(sorted_models):
            ...
            if dynamic_timeout_manager:
                ...
                if dynamic_timeout_manager.is_budget_exhausted():
                    print('⏰ Time budget exhausted, skipping remaining models')
                    break
            ...
            if best_results:
                ...
                print('   Checking if any result is better than currently deployed...')
            ...
            if model_name in self.best_configs:
                ...
                print(
                    f'📊 Training success rate: {successful_count}/{len(model_results)} ({training_success_rate:.1f}%)'
                )
            else:
...
Examples:
  ...
  python comprehensive_tuner.py --no-update-config        # Don't update config.json
    ...
    parser.add_argument('--models', '-m', help='Comma-separated list of models to tune (default: all enabled)')
    parser.add_argument(
        '--search-type',
        '-s',
        choices=['grid', 'random'],
    ...
    parser.add_argument(
        '--max-combinations', '-c', type=int, help='Maximum combinations per model (overrides default limits)'
    )
    parser.add_argument(
        '--unlimited', '-u', action='store_true', help='Remove all combination limits (full exploration)'
    )
    parser.add_argument(
        '--timeout-minutes', '-t', type=float, help='Maximum time per model in minutes (overrides default timeouts)'
    )
    parser.add_argument(
        '--unlimited-time', action='store_true', help='Remove all time limits (unlimited training time)'
    )
    parser.add_argument('--no-update-config', action='store_true', help="Don't update config.json with results")
    parser.add_argument('--show-failed', action='store_true', help='Show failed models list and exit')
    parser.add_argument('--clear-failed', action='store_true', help='Clear failed models list')
    parser.add_argument('--show-space', action='store_true', help='Show search space overview and exit')
    ...
    if search_type is None:
        ...
        active_models = {
            name: cfg
            for name, cfg in tuner.config['models'].items()
