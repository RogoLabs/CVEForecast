...
class BaseForecaster(ABC):
    ...
    def train_model(self, model_name: str, hyperparameters: Dict[str, Any],
                   train_data: TimeSeries, val_data: Optional[TimeSeries] = None) -> Tuple[Any, Dict[str, float]]:
        """
        ...
        try:
            ...
            if model is None:
                raise ValueError(f"Model creation returned None")
    ...
    def train_all_models(self, train_ratio: float = 0.8) -> Dict[str, Any]:
        ...
        series_df = training_series.pd_dataframe() if hasattr(training_series, 'pd_dataframe') else training_series.to_dataframe()
        ...
        if len(complete_months_df) < len(series_df):
            ...
            training_series = TimeSeries.from_dataframe(
                complete_months_df,
                freq=training_series.freq_str,
                fill_missing_dates=False
            )
            self.logger.info(f"Excluded current incomplete month from training. Using {len(training_series)} complete months (was {len(self.series)})")
            ...
            try:
                ...
                forecast_values = {
                    ...
                    for date, value in zip(
                        forecast_ts.time_index,
                        forecast_ts.values().flatten()
                    )
                }
