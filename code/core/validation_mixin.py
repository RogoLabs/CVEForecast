...
from validation.time_series_cv import RobustTimeSeriesValidator, format_cv_results
from validation.statistical_tests import ModelComparisonTests, format_comparison_results
from validation.interval_validation import IntervalValidator
...
class ValidationMixin:
    ...
    def perform_cross_validation(self, n_splits: int = 5, 
    ...
    def perform_statistical_tests(self, cv_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for _, row in rankings.iterrows():
            logger.info(
                f"  {row['rank']}. {row['model']:20s} "
                f"Avg Loss: {row['avg_squared_error']:.2f}  "
    ...
    def run_residual_diagnostics(self, models_to_test: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        if models_to_test is None:
            models_to_test = [name for name, data in self.model_results.items() 
    ...
    def run_horizon_analysis(self, horizons: List[int] = [1, 3, 6, 12],
        ...
        for model_name, model_data in top_models:
            ...
            try:
                model = self.create_model(model_name, model_data['hyperparameters'])
                results = analyzer.evaluate_by_horizon(
                    model,
                    self.series,
                    horizons=horizons,
                    n_splits=3
