...
class HorizonAnalysis:
    ...
    def evaluate_by_horizon(self, model, data: TimeSeries, 
    ...
    def _evaluate_single_horizon(self, model, data: TimeSeries, 
                                 horizon: int, n_splits: int) -> Dict[str, float]:
        """
    ...
    def compare_models_by_horizon(self, models_dict: Dict[str, Any],
                                  data: TimeSeries,
                                  horizons: List[int] = [1, 3, 6, 12]) -> pd.DataFrame:
        """
    ...
    def plot_horizon_comparison(self, comparison_df: pd.DataFrame,
                               metric: str = 'mape',
    ...
    def plot_single_model_horizon(self, horizon_results: Dict[str, Any],
        ...
        ax.errorbar(horizons, mapes, yerr=mape_stds, 
                   marker='o', linewidth=2, markersize=10,
                   capsize=5, capthick=2, label='MAPE ± std')
        
        # Add trend line
        if len(horizons) >= 2:
            z = np.polyfit(horizons, mapes, 1)
            p = np.poly1d(z)
            ax.plot(horizons, p(horizons), "--", alpha=0.5, 
                   label=f'Trend (slope={z[0]:.3f})')
        
        ax.set_xlabel('Forecast Horizon (periods)', fontsize=12, fontweight='bold')
        ax.set_ylabel('MAPE (%)', fontsize=12, fontweight='bold')
        ax.set_title(f'Forecast Accuracy by Horizon: {model_name}', 
...
def format_horizon_report(horizon_results: Dict[str, Any], 
