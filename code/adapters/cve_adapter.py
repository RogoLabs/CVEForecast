...
class CVEForecaster(BaseForecaster, ValidationMixin):
    ...
    def __init__(self, config_path: str = 'config.json'):
        ...
        self.forecast_tracker = ForecastTracker(
            history_path=config['file_paths'].get(
                'forecast_history',
        ...
        self.start_of_current_month = self.current_datetime.replace(
    ...
    def load_data(self) -> TimeSeries:
        ...
        self.forecast_constraints = ForecastConstraints(
            config=self.config,
            logger=self.logger
        )
        ...
        self.logger.info(f"✓ CNA momentum: {momentum_stats.get('current_cna_count', 0)} CNAs, "
    ...
    def _get_actuals_cumulative(self) -> List[Dict[str, Any]]:
        ...
        if not current_year_df.empty:
            ...
            for date, row in current_year_df.iterrows():
                ...
                next_month_first = pd.Timestamp(year=next_year, month=next_month_num, day=1, 
    ...
    def _generate_cumulative_timelines(self, forecasts: Dict[str, ForecastResult],
        ...
        if cumulative_timelines:
            ...
            if all_model_timelines:
                ...
                for date_str in all_dates:
                    totals = [item['cumulative_total'] for tl in all_model_timelines for item in tl if item['date'] == date_str]
                    if totals:
                        avg_timeline.append({
                            "date": date_str,
                            "cumulative_total": int(round(sum(totals) / len(totals)))
                        })
    ...
    def _generate_model_rankings_with_backtest(self, forecasts: Dict[str, ForecastResult], 
                                                 backtest_metrics: Dict[str, Dict[str, float]]) -> List[Dict[str, Any]]:
        """
    ...
    def _calculate_yearly_totals(self, cumulative_timelines: Dict[str, List[Dict[str, Any]]]) -> Dict[int, Dict[str, int]]:
        """
    ...
    def _calculate_forecast_vs_published(self, model_name: str) -> Tuple[List[Dict[str, Any]], Dict[str, float]]:
        ...
        try:
            ...
            self.logger.info(f"{model_name} backtest: MAE={summary_stats['mean_absolute_error']}, MAPE={summary_stats['mean_absolute_percentage_error']}%")
    ...
    def save_results(self, forecasts: Dict[str, ForecastResult]) -> str:
        ...
        for model_name in [m for m in forecasts.keys() if m != 'Ensemble']:
            ...
            forecast_vs_published[model_name] = {
        ...
        for date_str in sorted(all_forecast_dates):
            ...
            if values:
                ...
                ensemble_list.append({
                    "date": date_obj.strftime('%Y-%m'),
                    "cve_count": int(np.median(values))
                })
        ...
        self.logger.info(f"✓ Saved complete CVE forecast data:")
    ...
    def run_full_pipeline(self, 
        ...
        if run_validation:
            ...
            # Statistical tests
            if len(cv_results) >= 2:
                stat_results = self.perform_statistical_tests(cv_results)
