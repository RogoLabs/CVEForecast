...
class RobustTimeSeriesValidator:
    ...
    def __init__(self, n_splits: int = 5, min_train_size: int = 24, gap: int = 0, test_size: Optional[int] = None):
        """
    ...
    def validate_model(
        self, data: TimeSeries, model, forecast_horizon: int = 12, metric_name: str = 'MAPE'
    ) -> Dict[str, Any]:
        """
        ...
        tscv = TimeSeriesSplit(n_splits=self.n_splits, gap=self.gap, test_size=self.test_size or forecast_horizon)
        ...
        for fold_idx, (train_idx, test_idx) in enumerate(tscv.split(data.values())):
            ...
            if len(train_idx) < self.min_train_size:
                self.logger.debug(f'Fold {fold_idx}: Skipping (train size {len(train_idx)} < {self.min_train_size})')
                continue
            ...
            try:
                ...
                self.logger.debug(
                    f'Fold {fold_idx}: Train size={len(train)}, Test size={len(test)}, Origin={train.end_time()}'
                )
    ...
    def compare_models(self, data: TimeSeries, models: Dict[str, Any], forecast_horizon: int = 12) -> pd.DataFrame:
        """
