...
class ModelComparisonTests:
    ...
    def diebold_mariano_test(
        self, errors_a: np.ndarray, errors_b: np.ndarray, h: int = 1, alternative: str = 'two-sided'
    ) -> Dict[str, Any]:
        """
    ...
    def compare_multiple_models(
        self, errors_dict: Dict[str, np.ndarray], reference_model: Optional[str] = None
    ) -> pd.DataFrame:
        """
        ...
        if reference_model is None:
            ...
            for i, model_a in enumerate(models):
                for model_b in models[i + 1 :]:
                    dm_result = self.diebold_mariano_test(
                        errors_dict[model_a], errors_dict[model_b], alternative='two-sided'
                    )
            ...
            for model in models:
                ...
                dm_result = self.diebold_mariano_test(
                    errors_dict[reference_model], errors_dict[model], alternative='two-sided'
                )
    ...
    def model_confidence_set(self, errors_dict: Dict[str, np.ndarray], alpha: float = 0.10) -> Dict[str, Any]:
        """
        ...
        while len(remaining) > 1:
            ...
            avg_losses = {model: np.mean(losses[model]) for model in remaining}
            ...
            dm_result = self.diebold_mariano_test(
                errors_dict[worst_model], errors_dict[best_model], alternative='two-sided'
            )

            # If significantly worse, eliminate
            if dm_result['p_value'] < alpha and dm_result['mean_loss_diff'] > 0:
                self.logger.info(f'Eliminating {worst_model} (p = {dm_result["p_value"]:.4f})')
                remaining.remove(worst_model)
                eliminated_order.append(worst_model)
            else:
                # Cannot eliminate any more models
                self.logger.info(f'Cannot eliminate more models (p = {dm_result["p_value"]:.4f} >= {alpha})')
                break
        ...
        # Recommendation
        if len(included) == 1:
            recommendation = f'Use {included[0]} - clearly superior to other models'
        elif len(included) <= 3:
            recommendation = (
            ...
            recommendation = f'Use ensemble of top {len(included)} models. No single model is clearly best.'
    ...
    def rank_models_with_significance(self, errors_dict: Dict[str, np.ndarray]) -> pd.DataFrame:
        ...
        avg_losses = {name: np.mean(errors**2) for name, errors in errors_dict.items()}
        ...
        for rank, (model, loss) in enumerate(sorted_models, 1):
            ...
            if rank == 1:
                ...
                dm_result = self.diebold_mariano_test(
                    errors_dict[model], errors_dict[best_model], alternative='two-sided'
                )

                significantly_worse = dm_result['significant'] and dm_result['mean_loss_diff'] > 0
...
def format_comparison_results(dm_result: Dict[str, Any], model_a: str, model_b: str) -> str:
    """
