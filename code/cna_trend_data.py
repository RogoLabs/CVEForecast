        ...
        try:
            ...
            try:
                with open(cache_path, 'w') as f:
                    json.dump({'timestamp': datetime.now(timezone.utc).isoformat(), 'data': self.cna_data}, f)
    ...
    def get_monthly_cna_counts(self, start_year: int = 2015) -> pd.DataFrame:
        ...
        for date in months:
            ...
            if month % 3 == 1:  # First month of quarter
                mask = (df['join_year'] < year) | ((df['join_year'] == year) & (df['join_quarter'] <= quarter))
            else:  # Other months in quarter, use previous quarter
                prev_quarter = quarter if month % 3 != 1 else quarter - 1
                mask = (df['join_year'] < year) | ((df['join_year'] == year) & (df['join_quarter'] <= prev_quarter))

            cumulative_count = mask.sum()
            counts.append({'date': date, 'cna_count': cumulative_count})
